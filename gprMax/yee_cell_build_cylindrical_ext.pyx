...
#We don't have to change those function to adapt to the cylindrical coordinates as each time j-1 is called, there is a statement
# if j != 0.
...
cpdef void build_electric_components(np.uint32_t[:, :, ::1] solid, np.int8_t[:, :, :, ::1] rigidE, np.uint32_t[:, :, :, ::1] ID, G):
    ...
    # Er component
    componentID = G.IDlookup['Er']
    for i in range(0, G.nr_cyl):
        j = 0
        for k in range(1, G.nz_cyl):
            ...
            if get_rigid_Ex(i, j, k, rigidE):
                ...
                if numID1 == numID4:
                    ...
                    # Create magnetic does the same as averaging in 2 dimensions.
                    create_magnetic_average(i, j, k, numID1, numID4, componentID, G)

    # Ephi component
    componentID = G.IDlookup['Ephi']
    for i in range(1, G.nr_cyl):
        j = 0
        for k in range(1, G.nz_cyl):
            ...
            if get_rigid_Ey(i, j, k, rigidE):
                ...
                if numID1 == numID2 and numID1 == numID3 and numID1 == numID4:
                    ...
                    # Averaging is required
                    # We do have four components to average
...
cpdef void build_magnetic_components(np.uint32_t[:, :, ::1] solid, np.int8_t[:, :, :, ::1] rigidH, np.uint32_t[:, :, :, ::1] ID, G):
    ...
    # Hphi component: no averaging is needed as we only have 2 dimensions...
